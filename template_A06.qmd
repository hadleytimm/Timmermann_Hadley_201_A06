...
max_ind <- which.max(median_temps)
min_ind <- which.min(median_temps)

max_temp <- median_temps[max_ind]
min_temp <- median_temps[min_ind]

max_temp_year <- max_temps_CO$year[max_temp]
min_temp_year <- max_temps_CO$year[min_temp]
...
sum_of_digits <- function(number){
  if(number == 0){ # base case
    ...
    trunc_num <- number %/% 10
    sum_num <- last_digit + sum_num
    return(sum_num + sum_of_digits(trunc_num))
  }
}
