...
```{r}
points_per_game <- lapply(player_info, function(x) {x[[4]]/x[[3]]})
points_per_game
```
...
#for(i in names(player_info)){
 ...
 # points_pg_list <- append(points_pg_list, player$points/player$games_played)
#  points_pg_list$player <- points_per_game
#}
...
#for(i in 1:length(player_info)){
 # player_name <- names(player_info[[i]])
 # points_per_game = player_info[[i]]$points/player_info[[i]]$games_played
 # points_pg_list$player_info[[i]] <- points_per_game
#}
