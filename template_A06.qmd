...
[https://github.com/hadleytimm/Timmermann_Hadley_201_A06](https://github.com/hadleytimm/Timmermann_Hadley_201_A06)
...
```{r}
player_info[[2]][[4]] # index with index
player_info$LJ$points # index with name
...
```{r}
heights <- list(MJ = 78, LJ = 81, KB = 78, KA = 86, SO = 85) # create list of heights

for (i in 1:length(player_info)){ # for # of lists in <player_info> lists
   # index <player_info> each time / assign height variable to the height from list heights aquired by using same index
  player_info[[i]]$height <- heights[[i]] 
 
}

print(player_info) # print the list with height variable added
```
### Part c (3 points)
...
```{r}
points_per_game <- list() # initialize list

# create a function to calculate points per game and use lapply to apply to specified <player_info> variables

points_per_game <- lapply(player_info, function(x) {x$points/x$games_played})
points_per_game # print
```
### Part d (1 point)
...
```{r}
# use sapply to round each item in <points_per_game>
points_vec <- sapply(points_per_game, round, 2)
points_vec # print vector
```
...
```{r}
round_func <- function(x){ # define function and inputs
  if (x >= 10 && x < 100){ # if x is greater than or equal to 10 and less than 100
    return(round(x, -1)) # round to the nearest 10th
  }else if(x >= 100 && x < 1000){ # else if x is greater than or equal to 100 and less than 1000
    return(round(x, -2)) # round to the nearest hundred
  }else{ # x is greater than 1000
    return(round(x, -3)) # round to the nearest thousand
  }
}

# use apply function to apply <round_func> to each matrix entry
...
```{r}
# use apply to apply mean function to columns 3-14 (months)
...
# slice <max_temps> by ($state == <insert state>), store all TRUE in new variable
...
```{r}
# apply median function by row to <max_temps_CO, columns 3-14 (months)> using apply
median_temps <- apply(max_temps_CO[3:14], 1, median)

max_ind <- which.max(median_temps) # store index for max median position
min_ind <- which.min(median_temps) # store index for min median position

max_temp <- median_temps[max_ind] # store value for max median using index
min_temp <- median_temps[min_ind] # store value for min median using index

# store year of max median by indexing with max
max_temp_year <- max_temps_CO$year[max_ind] 
# store year of min median by indexing with min
min_temp_year <- max_temps_CO$year[min_ind]

# print statement using variable
print(paste0("The maximum median annual temperature of ", max_temp, " occurred in ", max_temp_year, " and the minimum median annual temperature of ", min_temp, " occurred in ", min_temp_year, "."))
...
sum_of_digits <- function(number){ # define function and inputs
  if(number < 10){ # base case
    return(number) # sum is number
  }else{ # recursive case
    last_digit <- number %% 10 # obtain last digit
    trunc_num <- number %/% 10 # truncate number to exclude last digit
    sum_num <- last_digit + sum_num # add last digit to current sum
    return(sum_num + sum_of_digits(trunc_num)) # keep sum and run function again with truncated number
  }
}

sum_of_digits(935068) # run function with input
```
